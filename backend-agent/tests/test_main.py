...
def test_chat_endpoint_token_limit(client, mock_auth_user, mocker):
    ...
    mocker.patch(
        "main.validate_token_count",
        side_effect=HTTPException(status_code=400, detail="Message too long"),
    )

    response = client.post(
        "/chat", json={"session_id": "123", "message": "A very long message..."}
    )
...
def test_chat_endpoint_internal_error(client, mock_auth_user, mocker):
    ...
    mocker.patch(
        "main.protected_graph_invoke", side_effect=Exception("Something went wrong")
    )
