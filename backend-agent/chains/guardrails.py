...
class SecurityBlocker:
    ...
    def __init__(self):
        ...
        self.patterns = [
            re.compile(p, re.IGNORECASE | re.MULTILINE) for p in self.all_patterns
        ]
    ...
    def is_malicious(self, content: str) -> bool:
        ...
        for pattern in self.patterns:
            if pattern.search(content):
                logger.warning(
                    f"Regex Security Block triggered by pattern: {pattern.pattern}"
                )
                return True
        return False
...
judge_prompt = ChatPromptTemplate.from_messages(
    [
        (
            "system",
            """You are a specific automated security guardrail.
